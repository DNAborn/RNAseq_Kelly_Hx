---
title: "RNA-Seq Kelly all (original sample names)"
author: "Kelterborn"
date: "2024-02-21"
output:
  md_document:
...
rowRanges(gse) %>% kable() %>% kable_styling("striped", full_width = F) %>% scroll_box(height = "400px")
...
```{r pre_sample_dist, fig.height=12}
...
SYMBOLs %>% length()
ENSEMBLE <- as.character(mapIds(edb,keys = SYMBOLs, column = "GENEID", keytype = "SYMBOL", multiVals="first"))
...
SYMBOLs <- data.frame(SYMBOL = SYMBOLs,
                      ENSEMBL = mapIds())
rownames(SYMBOLs) <- SYMBOLs

  
s2e <- data.frame(mcols(dds)[,c("gene_id","SYMBOL")])
s2e %>% dplyr::filter(complete.cases(.)) %>% na.omit()
rownames(s2e) <- s2e$SYMBOL

SYMBOLs <- s2e$SYMBOL %>% unique() %>% na.omit()
SYMBOLs <- SYMBOLs[!(SYMBOLs == "")]
s2e

plotCounts(dds,gene="CA9")

all_counts <- {}
for (i in 1:l){
  d <-  plotCounts(dds, gene=goi[i,"gene_id"], intgroup="condition",main=res$SYMBOL[i],returnData=TRUE)
