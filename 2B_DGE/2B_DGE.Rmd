...
ev_f <- EnhancedVolcano(res,
    ...
    y = 'padj',
    lab = res$symbol,
    selectLab  = res[res$islol == TRUE | res$isl1 == TRUE | res$isl2 == TRUE,"symbol"],
    labSize = 2,
    drawConnectors = TRUE,
    boxedLabels = TRUE,
    widthConnectors = 0.5,
...
```{r wgcna_correlation, eval=TRUE, echo=FALSE, fig.show='hold',out.width=33%}

# correlation with network analysis
pie(mcols(dds)[input_list[[1]],"colors"] %>% .[!is.na(.)] %>% summary(),col=mcols(dds)[input_list[[1]] %>% rownames(),"colors"] %>% summary() %>% names(), main="Correlation with WGCNA (combined groups")
pie(mcols(dds)[input_list[[2]],"colors"] %>% .[!is.na(.)] %>% summary(),col=mcols(dds)[input_list[[2]] %>% rownames(),"colors"] %>% summary() %>% names(), main="Correlation with WGCNA (combined groups")
pie(mcols(dds)[input_list[[3]],"colors"] %>% .[!is.na(.)] %>% summary(),col=mcols(dds)[input_list[[3]] %>% rownames(),"colors"] %>% summary() %>% names(), main="Correlation with WGCNA (combined groups")
...
### Vulcano lists
```{r vulcano_lists, eval=TRUE, echo=FALSE}
...
goi <- results_list[[1]][results_list[[1]]$symbol=="EPO",] %>% rownames()
...
# correlation with network analysis
pie(mcols(dds)[top_hif1b %>% rownames(),"colors"] %>% .[!is.na(.)] %>% summary(),col=mcols(dds)[top_hif1b %>% rownames(),"colors"] %>% summary() %>% names(), main="Correlation with WGCNA")
...
### - overlap
```{r venn_overlap, echo=FALSE, fig.height=4}
