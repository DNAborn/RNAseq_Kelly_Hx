...
```{r 2_venn, fig.height=6, fig.width=8, dpi=300, out.width="50%", fig.show='hold'}
...
```{r 2_venn2, fig.height=6, fig.width=8, dpi=300, out.width="50%", fig.show='hold'}
...
```{r master_table_combine, eval=FALSE}
...
new_names <- paste(meta$condition)

colnames(counts_norm)[-1] <- make.unique(new_names, sep="_")

print(head(colnames(counts_norm)))

counts_norm <- counts_norm %>%
  select(
    gene_id,                # ID immer zuerst
    matches("^Kelly_Nx"),   # Alle Kelly Normoxie Replikate
    matches("^Kelly_Hx"),   # Alle Kelly Hypoxie Replikate
    matches("^HIF1A_Nx"),  
    matches("^HIF1A_Hx"),
    matches("^HIF2A_Nx"),
    matches("^HIF2A_Hx"),
...
# combine counts with master table
if(!"gene_id" %in% colnames(counts_norm)) {
  counts_norm <- counts_norm %>% rownames_to_column("gene_id")
}
colnames(counts_norm)[1] <- "Ensembl"

counts(dds, normalized=TRUE) %>% 
  as.data.frame() %>%
  rownames_to_column("gene_id") %>% .[c(3),c(1,24)]

colData(dds)[23,c("condition","filename")]

counts_norm[3,c(1,34:40)]


# 2. Zusammenfügen (Join)
master_counts_table <- left_join(master_table, counts_norm, by = "Ensembl")

write.xlsx(master_counts_table,"master_counts_table.xlsx")



# Filter for ML

master_counts_table %>% colnames()
...
# Master Table Dive
## Venn
```{r master_table, fig.show='hold', out.width="50%"}
master_table <- {}
master_table <- read.xlsx("/mnt/s/AG/AG-Scholz/Neuroblastoma_HIF/RNA-Seq data/Own data/master_table.xlsx")
rownames(master_table) <- master_table$Ensembl
