...
# update 29.6.2026 Index Ensembl 116
...
indexparentdir="/mnt/s/AG/AG-Scholz-NGS/Daten/Salmon/index"
...
base_url="http://ftp.ensembl.org/pub/release-${release}/fasta/homo_sapiens"

# 1. Download der genomischen Daten (Release 116)
base_url="http://ftp.ensembl.org/pub/release-${release}/fasta/homo_sapiens"

wget -P "$gdir" "${base_url}/dna/Homo_sapiens.GRCh38.dna.primary_assembly.fa.gz"
wget -P "$gdir" "${base_url}/cdna/Homo_sapiens.GRCh38.cdna.all.fa.gz"
wget -P "$gdir" "${base_url}/ncrna/Homo_sapiens.GRCh38.ncrna.fa.gz"
wget -P "$gdir" "http://ftp.ensembl.org/pub/release-${release}/gff3/homo_sapiens/Homo_sapiens.GRCh38.${release}.gff3.gz"

# 2. Transkriptom zusammenführen
echo "Kombiniere cDNA und ncRNA zum kompletten Transkriptom..."
cat Homo_sapiens.GRCh38.cdna.all.fa.gz Homo_sapiens.GRCh38.ncrna.fa.gz > transcriptome_complete.fa.gz

# 3. Decoy-Liste erstellen
echo "Extrahiere Decoy-Header..."
zgrep "^>" Homo_sapiens.GRCh38.dna.primary_assembly.fa.gz | cut -d " " -f 1 | sed 's/>//g' > decoys_human_ensh38.txt

# 4. Gentrome erstellen
echo "Kombiniere Sequenzen zum Gentrome..."
cat transcriptome_complete.fa.gz Homo_sapiens.GRCh38.dna.primary_assembly.fa.gz > gentrome_human_ensh38.fa.gz

# 5. Salmon Index erstellen

# Salmon v2 (hatte Probleme mit vollem decoy aware index)
mamba create -n salmon2 salmon
mamba activate salmon2
salmon --version
# salmon 2.2.1

# Zurück zu Salmon v1
mamba activate salmon
# mamba install "salmon=1.12.1"
mamba install "salmon<2"
# salmon 1.12.1

indexname="${species}_ens${release}_index"
echo "Starte Salmon Indexing: $indexname"

# indexparentdir="/mnt/s/AG/AG-Scholz-NGS/Daten/Salmon/index"

indexdir="$indexparentdir/$indexname"
# rm -rf $indexdir
mkdir -p $indexdir
# export LC_ALL=C
LC_ALL=C LANG=C salmon index -t gentrome_human_ensh38.fa.gz -d decoys_human_ensh38.txt -p 20 -i $indexdir
salmon index -t gentrome_human_ensh38.fa.gz -d decoys_human_ensh38.txt -p 20 -i $indexdir


# Linux
~/Downloads/salmon-linux-x86_64/bin/salmon index \
 -t gentrome_human_ensh38.fa.gz \
 -d decoys_human_ensh38.txt \
 -p 20 \
 -i $indexdir
...
# Mac
salmon index \
  -t "/Users/simonkelterborn/Library/CloudStorage/OneDrive-Charité-UniversitätsmedizinBerlin/! Files 1/Manuscripts/Kelly hypoxia/Ens116/gentrome_human_ensh38.fa.gz" \
  --decoys "/Users/simonkelterborn/Library/CloudStorage/OneDrive-Charité-UniversitätsmedizinBerlin/! Files 1/Manuscripts/Kelly hypoxia/Ens116/decoys_human_ensh38.txt" \
  -i "/Users/simonkelterborn/Library/CloudStorage/OneDrive-Charité-UniversitätsmedizinBerlin/! Files 1/Manuscripts/Kelly hypoxia/Ens116/human_ens116_index" \
  -p 6
...
fastqfiles=("$fastqdir"/*R1*.fastq.gz)
fastqfiles2=("$fastqdir2"/*R1*.fastq.gz)
fastqfiles3=("$fastqdir3"/*R1*.fastq.gz)

fastqfiles_all=("${fastqfiles[@]}" "${fastqfiles2[@]}" "${fastqfiles3[@]}")

echo "Gefundene R1-Dateien: ${#fastqfiles_all[@]}"

# Sicherer Ordneraufruf
mkdir -p "$quantdir"

for R1 in "${fastqfiles_all[@]}"; do
    # Dateinamen und Sample-Namen extrahieren
    bfn=$(basename "$R1")
    samp=${bfn%"_R1_001.fastq.gz"}
    
    # Präziser R2-Austausch (schützt vor 'R1' im Ordnernamen)
    R2="${R1/_R1_001.fastq.gz/_R2_001.fastq.gz}"
    
    echo "Processing Sample: $samp"
    
    # Check ob beide Dateien existieren, sonst überspringen
    if [[ ! -f "$R1" || ! -f "$R2" ]]; then
        echo "Fehler: $R1 oder $R2 fehlt. Überspringe Sample..."
        continue
    fi
    ...
    # Der optimierte Salmon v1.12 quant Aufruf
...
echo ""
echo "========================================="
echo "Mapping abgeschlossen. Starte Überprüfung..."
echo "========================================="

# Die Anzahl der R1-Dateien ist unser Zielwert
expected_count=${#fastqfiles_all[@]}
success_count=0
missing_samples=()

for R1 in "${fastqfiles_all[@]}"; do
    # Sample-Namen exakt wie in der Hauptschleife extrahieren
    bfn=$(basename "$R1")
    samp=${bfn%"_R1_001.fastq.gz"}
    
    # Der erwartete Pfad zur finalen Output-Datei
    expected_sf="${quantdir}/${samp}_quant/quant.sf"
    
    # Prüfen, ob die Datei existiert und größer als 0 Byte ist
    if [[ -s "$expected_sf" ]]; then
        ((success_count++))
    else
        missing_samples+=("$samp")
    fi
done

echo "Erwartete Quantifizierungen: $expected_count"
echo "Erfolgreich abgeschlossen:   $success_count"
echo ""

if [[ $success_count -eq $expected_count ]]; then
    echo "✅ PERFEKT! Alle $expected_count Proben wurden vollständig und fehlerfrei gequanted."
else
    echo "❌ ACHTUNG! Es fehlen $((expected_count - success_count)) Proben (oder sie sind unvollständig)."
    echo "Bitte überprüfe die Logs für folgende Samples:"
...
sample_table_all$experiment <- sample_table_all$experiment %>% as.factor() %>% relevel(ref="Katharina")
...
### Plot mapping rates
```{r plot_mappingr, fig.height=10, include=TRUE, echo=FALSE}
...
text(x = mappingrates, y = xx, label = mappingrates, pos = 4, cex = 0.2, col = "red")
...
# columns with unique symbols
mcols(gse)$ens.symbol <- mcols(gse)$gene_name
mcols(gse)$ens.symbol[mcols(gse)$ens.symbol == ""] <- mcols(gse)$gene_id[mcols(gse)$ens.symbol == ""]
mcols(gse)$ens.symbol[duplicated(mcols(gse)$ens.symbol)] <- mcols(gse)$gene_id[duplicated(mcols(gse)$ens.symbol)]
...
data
# [1] "/mnt/s/AG/AG-Scholz-NGS/Daten/Simon/RNA-Seq_Kelly_all/data"
# load(file=paste(data,"tximeta.txm", sep="/"))
...
### (- remove outlier)
DEseq2 with all samples, design = ~experiment + genotype + treatment + genotype:treatment
-> outlier: 2480, dispOutlier: 681
DEseq2 with all samples, design = ~genotype + treatment + genotype:treatment
...
```{r dds_design, fig.show='hold', out.width="50%", fig.height=5, fig.width=5}
design(dds)
summary(results(dds, name="treatment_Hx_vs_Nx"))
plotMA(dds)
plotDispEsts(dds)
...
```{r deseq2 without outlier, eval=FALSE}
...
column_dend <- as.dendrogram(hclust(sampleDists))
...
colnames(vsd) <- vsd$names
sampleDistMatrix <- dist(t(assay(vsd))) %>% as.matrix()
mat <- sampleDistMatrix

## Color
col_fun = colorRamp2(c(-2, 0, 2), c("blue", "white", "red"))
col_fun <- colorRamp2(seq(quantile(mat,0.05),quantile(mat,0.95), length = 100), rev(viridis(100,option = 'viridis')))

## TOP annotation
n.col <- ncol(sampleDistMatrix)
column_ha = HeatmapAnnotation(df = as.data.frame(colData(vsd)[,c("experiment","treatment","genotype")]),
                              col = list(treatment = c("Nx" = "lightcoral", "Hx"="skyblue1"),
                    genotype = c("Kelly" = "grey","HIF1A"="seagreen3","HIF2A"="turquoise3","HIF1B"="tan2"),
                    experiment = c("Katharina"=viridis(4, option="plasma")[1],
                    "Control"=viridis(4, option="plasma")[2], 
                    "Simon"=viridis(4, option="plasma")[3],
                    "Ulrike"=viridis(4, option="plasma")[4]) ))

## Dendogramm
# method = 'pearson'
col_dend <- hclust(dist(t(mat)), method = 'single')
col_dend <- dendsort(col_dend)
...
```{r pca, warning = FALSE, fig.show="hold", out.width="80%", fig.height=6, eval=TRUE, include=TRUE, echo=FALSE}
# transform data
# load(file=paste(data,"deseq2.dds", sep="/"))
...
screeplot(p,
    ...
    geom_label(aes(x = horn$n + 1, y = 30,
      label = paste("Horn\'s=",horn$n), vjust = -1, size = 8)) +
    geom_label(aes(x = elbow + 1, y = 50,
      label = paste("Elbow=",elbow), vjust = -1, size = 8))
...
rowVars(assay(vsd)) %>% head()

vsd <- vsd[order(rowVars(assay(vsd))),]
i <- 10
saveGIF({
...
n <- i*1000
vsd_v <- vsd[c(n:(n+999)),]
print(paste(n," - ",n+1000))

pcaData <- plotPCA(vsd_v, intgroup=colnames(colData(vsd)), returnData=TRUE,ntop=1000)
percentVar <- round(100 * attr(pcaData, "percentVar"))
g1 <- ggplot(pcaData, aes(PC1, PC2, color=treatment, shape=genotype)) +
  geom_point(size=5, alpha=0.7) +
  labs(title = "treatment") +
...
g3 <- ggplot(pcaData, aes(PC1, PC2, color=condition, shape=genotype)) +
  ...
  scale_color_viridis_d(option ="viridis")
plot((g1+g2)/g3 +
       plot_annotation(title = paste("number of variable genes: ",n," - ",n+1000)) +
       plot_layout(guides = 'collect')
     )
ani.pause()
}
}, interval = 1, movie.name = file.path(gitdir,"1_data_processing/pca.gif"), ani.width = 600, ani.height = 600)

```

###### -- #PCA gif2
```{r pca_gif2, fig.height=8, fig.show="hold", warning=FALSE, out.width="100%", eval=FALSE, include=FALSE, echo=FALSE}
...
}, interval = 0.3, movie.name = file.path(gitdir,"1_data_processing/pca2.gif"), ani.width = 600, ani.height = 600)
...
[PCA Gif](pca2.gif){height=100%, width=100%}
...
gcounts3 <- ggplot(all_counts[all_counts$Gene=="CA9",], aes(x = condition, y = count, fill=treatment, color=treatment)) +
...
gcounts4 <- ggplot(all_counts[all_counts$Gene=="EPO",], aes(x = condition, y = count, fill=treatment, color=treatment)) +
  geom_boxplot(color="black") +
  geom_point(shape=21,color="black",aes(fill=experiment),position=position_dodge(width=0.75), alpha=1) +
  scale_fill_manual(values=c(cols[c(1,5)],viridis(4))) +
  scale_color_manual(values=cols[c(1,5)]) +
